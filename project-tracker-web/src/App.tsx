...
export default function App() {
  ...
  const [initializing, setInitializing] = useState(true); // Защита от мигания интерфейса

  // ЖИВАЯ ПРОВЕРКА СЕССИИ ПРИ ЗАПУСКЕ
  useEffect(() => {
    const checkSession = async () => {
      try {
        // const res = await axios.get('https://localhost:7291/api/auth/me', { withCredentials: true });
        const res = await axios.get('/api/auth/me', { withCredentials: true });

        setCurrentUser(res.data); // Сессия жива, сохраняем юзера
      } catch (err) {
        setCurrentUser(null); // 401 Unauthorized или ошибка бэка -> разлогинен
      } finally {
  ...
  />
          ...
          <EmployeeManager />
        ) : view === 'create' ? ( // <-- ДОБАВЛЯЕМ ЭТО УСЛОВИЕ
          <WizardProvider>
            <ProjectWizard
              onSuccess={() => setView('dashboard')}
              onCancel={() => setView('dashboard')} // <-- Добавляем обработчик
            />
          </WizardProvider>
